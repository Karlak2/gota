...
export class CountingDetails extends Component {        /*  */
    setText=()=>{
        let ar=[]
        let a=this.props.current
        let b=this.props.upgradeTypes[a].act
        let c=this.props.upgradeTypes[a].max
        let d=this.props.upgradeTypes[a].required
        if(d>this.props.upgradeLevel){
            console.log(d,this.props.upgradeLevel)
            ar.push(<h2 key="det1" className="headText">{this.props.upgradeTypes[a].head}</h2>)
    ...
    coloring=()=>{
        ...
        let b=this.props.upgradeTypes[a].act    /*Actual selected upgrade actual level */
        let p=this.props
        let colorcheck=true
    ...
    buttonSee=()=>{
        ...
        return{
            display:p.upgradeTypes[a].act===p.upgradeTypes[a].max||
            p.upgradeTypes[a].required>p.upgradeLevel||
            p.yetUpgrade!=""||
            !chack||!check? "none":"flex"
        }
    }
    add=()=>{
        this.props.upgrade(this.props.current)
    }
    upgradeCost=()=>{
        let sel=this.props.current
        let c=[]
        const {act,max}=this.props.upgradeTypes[sel]
        if(act<max){
        ...
        return (
            <div className="upgradeDetails">
                <div className="upgradeType">
                    <img src={this.props.upgradeTypes[this.props.current].pics} 
                        alt="Upgrade"
                        width="120px"
                        height="120px">
                    ...
                    <div className="upgradeBox">
                        ...
                        <p style={{display:'inline-flex'}}>
                            <span>{this.props.upgradeTypes[this.props.current].act}</span>/<span>{this.props.upgradeTypes[this.props.current].max}</span>
                        </p>
                    </div>
                    <button 
                     className="addButton" 
...
export default CountingDetails
